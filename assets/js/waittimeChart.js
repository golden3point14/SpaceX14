...
$('#waitchart-button').css('background-color', '#315B7E');
...
function useDatabaseData() {
  for (var i=0; i<JSONtasks.length; i++)
  {
      if (JSONtasks[i].name !== "<idle>") {
        ...
        if (JSONtasks[i].totalWaittime > maxWaittime)
        {
          maxWaittime = JSONtasks[i].totalWaittime;
        }

        if (JSONtasks[i].totalWaittime < minWaittime)
        {
          minWaittime = JSONtasks[i].totalWaittime;
        }
