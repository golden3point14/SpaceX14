...
software. These cyclical patterns of processes are called {\bf cycles}.

SpaceX currently uses a tool called KernelShark
visualize trace data and help find any bugs or anomalies
(Figure~\ref{fig:kernelshark},
\url{http://people.redhat.com/srostedt/kernelshark/HTML/}).  SpaceX engineers look for anomalies in process
timing, which often indicates software problems on the rocket. We were informed
by our liaisons that this debugging process can take up to several days, which
is an inefficient use of engineers' time. \\

\begin{figure}[H]
  \centering
      \includegraphics[width=4.95in]{kshark-open.png}
      \caption{\label{fig:kernelshark}KernelShark consists of a graphical display of the programs running on a computer over time (top half of screen), as well as a tabular view of the same information, in the form of CPU events such as tasks switching, waking, or sleeping (bottom half of screen).}
\end{figure}
...
\section{Obtaining and Running SpaceShark}
    ...
    \begin{tabular}{p{0.2\linewidth}p{0.7\linewidth}}
     \toprule
      ...
      \multirow{2}{*}{Task Statistics} & Give a user high-level statistics 
        about the data, comparing tasks' total runtimes and preemptions\\
...
\subsection{Example User Flow}
  ...
  table of preemptions, or when hovering over a preemption event in the chart
  below it.
  ...
  user can then hover over this sequence on the chart and see which tasks were
...
\begin{figure}[H]
  \centering
      ...
      \caption{\label{fig:load_file}Choose File selects a new file and Use Last Data selects the file
  that was most recently loaded into the tool. The Use Last Data option will not
display if no data has ever been loaded.}
  \end{figure}

\paragraph{Purpose:}
When the user opens SpaceShark, they see the Load File page
(Figure~\ref{fig:load_file}).  This page allows the user to choose which data to
load, or to choose to load the data that was most recently used.  The user can
easily resume their work by choosing the second option, as both the data and the
state of each page, such as the amount that the user has scrolled or zoomed in
on the visualizations, from the previous session are reloaded.
...
\paragraph{Interactivity:} The user can zoom in and out on the chart by using
the mouse scroll wheel, and pan left and right by clicking and dragging.
Double-clicking on the chart also zooms in on it. The user can hover over any
event on the chart to see a text box with more information about that particular
...
sleeping). Clicking on an event displayed in the chart will cause the
...
\paragraph{Comparison to KernelShark:} 
  ...
  KernelShark also has chart plots of the CPUs and includes similar features. A
  user can zoom by clicking and dragging a rectangular box from left to right
  over the chart, which allows the user to precisely view a specific area of the
  chart.  KernelShark's zoom functionality doesn't follow common zoom functionalities of
  a modern-day desktop application. Zooming out of the chart in KernelShark is
  also unintuitive: the user clicks and drags a rectangular box from right to
  left over the chart. This technique does not give the user any kind of
  information about how much the chart is zooming out and can therefore be
  confusing to users. When zooming on KernelShark, a horizontal bar appears at
  the bottom of the chart to be able to easily pan.
  ...
  chart and have the table scroll to that event.  Users can hover over the chart
  ...
  In KernelShark, the user can select individual tasks to plot on the same chart
...
%    gantt-chart-d3.js and dataTables.scroller.js, respectively.
  ...
  \caption{The Cycles page displays charts of each CPU. The user can define
...
top allow the user to select which CPUs to display.}
  ...
  chart for unusual behavior.

\paragraph{Interactivity:}    
    Like the chart on the Overview page, the user can zoom and pan the charts on
    the cycles page.  The page also has checkboxes to toggle on and off each
    CPU, which removes or adds the charts for these CPUs, rescaling other charts
    ...
    collected.  If not, the charts will not appear when the page first loads.
    In either case, the user can enter a number in the top box to set the period
    of desired cycles, which will then display a chart of cycles of that length.
...
\paragraph{Interactivity:}
All bars in the chart have distinct colors to represent different tasks. These
colors are consistent with the color scheme throughout the rest of the
application. The user can click on any task in the bar chart and will be
redirected to the Task State page for more information about that task.
The user can toggle between viewing the preemption count chart and the runtime
chart by clicking the small thumbnails in the sidebar.
...
statistics about the data, so it does not display any statistics about the data.
  ...
  that preempted the current task. The bottom half is a chart that plots the
...
\paragraph{Interactivity:}
    The chart on the bottom half of the page functions the same as the charts on
    the Overview, Cycles, and Compare Tasks page
    . The user can click the ``+ Compare'' button at the bottom
    to add this task chart to a series of side-by-side charts for comparison.
    
\paragraph{Comparison to KernelShark:}
     KernelShark can display a chart of a single task over time, however it does not show which other tasks preempted a given task, nor how many times it was preempted.
  
  \subsection{Compare Tasks Page}
  
  \begin{figure}[H]
  \includegraphics[scale=0.3]{compare-page.png}
  \caption{The Compare Tasks page vertically displays the Task State charts for
  multiple tasks. They can be added, reordered, or removed.}
  \end{figure}

\paragraph{Purpose:}
The Compare Tasks page allows users to view the state charts for multiple tasks. Tasks can be reordered so that specific tasks can
be compared side by side for the same time period. 

\paragraph{Interactivity:}
Users can plot charts by either (1) typing in a task name or PID in an autocomplete box at the top of the page, or
(2) viewing a task on the Task State page and clicking the ``+ Compare''
button. The user can
rearrange the order of the charts on the page by dragging them up or down. Each
chart can be removed by clicking on the ``Remove Task'' button next to it.
    
\paragraph{Comparison to KernelShark:}
    In KernelShark, the user can view a list of tasks and select which ones they
    want to chart.  This list of tasks is not searchable and may be very long.
...
way, for example by adding a new feature or making a change to the type of data that SpaceShark can display.
  ...
  Figure~\ref{fig:architecture} provides a visual representation of the flow of our application
  while also displaying its modular parts.
  
  \begin{figure}[H]
    \begin{center}
    \includegraphics[scale=0.15]{architecture_diagram.png}
    \caption{ \label{fig:architecture} A visual representation of the three components of our application. Each
 ...
 which captures various events in the Linux kernel while it runs. The user specifies the kinds of events \texttt{trace-cmd} should record. SpaceShark users are interested in scheduling events: what tasks were
  ...
  report of this record. An example line appears in
  Figure~\ref{fig:example_lines}.
...
varies based on the type of event.
  ...
  whitespace. Then it extracts fields one by one. For the
...
The output JSON has the elements described in Table~\ref{table:json_elements}.
As noted, several of these JSON objects contain other JSON objects: \texttt{tasks},
\texttt{events}, and \texttt{cycleEvents}. 
...
\begin{table}
  ...
  \begin{center}
    \begin{tabular}{p{0.35\linewidth}p{0.55\linewidth}}
      ...
      \midrule
      \multirow{2}{*}\texttt{Tasks}      & An array of all tasks seen in the trace\\
      \multirow{2}{*}\texttt{Events}     & An array of all events seen in the trace\\
      \multirow{2}{*}\texttt{cycleEvents} & An array of cycle events generated from user markers\\
      \multirow{2}{*}\texttt{numCPUs}     & The number of CPUs in the trace\\
      \multirow{2}{*}\texttt{AutocompleteNames} & An array of strings, listing all tasks names\\
      \multirow{2}{*}\texttt{AutocompleteEventTypes} & An array of strings, listing all event types in the trace\\
      \bottomrule
    \end{tabular}
    \vspace{-1em}
    \caption{\label{table:json_elements} A list of output JSON elements}
  \end{center}
\end{table}

A \texttt{task} has the data described in Table~\ref{table:tasks}.
...
\begin{table}
  ...
  \begin{center}
    \begin{tabular}{p{0.25\linewidth}p{0.2\linewidth}p{0.45\linewidth}}
      ...
      description} \\
      \midrule
      \multirow{3}{*} \texttt{name}            & string       & \\
      \multirow{3}{*} \texttt{pid}             & int          & \\
      \multirow{3}{*} \texttt{events}          & int array    & Indices into the Events array to find associated events\\
      \multirow{3}{*} \texttt{preemptedBy}     & string array & Names of tasks that preempted this task \\
      \multirow{3}{*} \texttt{preemptionCount} & int          & A running total of times this task was preempted                    \\
      \multirow{3}{*} \texttt{totalRuntime}    & long         & Time this task ran in milliseconds during the trace        \\
      \multirow{3}{*} \texttt{totalWaittime}   & long         & Time this task waited in milliseconds during the trace     \\
      \multirow{3}{*} \texttt{totalSleeptime}  & long         & Time this task slept in milliseconds during the trace\\
      \bottomrule
    \end{tabular}
  \end{center}
  \vspace{-1em}
  \caption{\label{table:tasks} Task data format}
\end{table}

  An \texttt{event} contains the data in Table~\ref{table:events}. Switch events contain some additional
...
\begin{table}
  ...
  \begin{center}
    \begin{tabular}{p{0.2\linewidth}p{0.1\linewidth}p{0.6\linewidth}}
      ...
      \multirow{3}{*} \texttt{name}      & string &                                        \\
      \multirow{3}{*} \texttt{pid}       & int    &                                        \\
      \multirow{3}{*} \texttt{cpu}       & int    &                                        \\
      \multirow{3}{*} \texttt{startTime} & double & Time in milliseconds since kernel boot \\
      \multirow{3}{*} \texttt{eventType} & string &                                        \\
      \multirow{3}{*} \texttt{extraInfo} & string &                                        \\
      \multirow{3}{*} \texttt{activeName}* & string & The name of the task switching in      \\
      \multirow{3}{*} \texttt{activePID}*  & string &                                        \\
      \bottomrule\\
\multicolumn{3}{l}{ *This information is only present on switch events.}
    \end{tabular}
  \end{center}
  \vspace{-1em}
  \caption{\label{table:events}Event data format}
\end{table}
  ...
  \texttt{CYCLE\_START}, then we create a \texttt{cycleEvent} that contains the data described
  in Table~\ref{table:print_events}.
...
\begin{table}
  ...
  \begin{center}
    \begin{tabular}{lll}
      ...
      \midrule
      \multirow{2}{*}\texttt{startTime}      & double                               \\
      \multirow{2}{*}\texttt{extraInfo}       & string                              \\
      \bottomrule
    \end{tabular}
  \end{center}
  \vspace{-1em}
  \caption{\label{table:print_events}Print event data format}
\end{table}
  ...
  \subsection{Node-Webkit and HTML} %Rachel
    ...
    extending KernelShark, the existing tool. After looking at KernelShark's
    code,
    a large number of C files, we decided it would be faster to
...
\begin{center}\url{https://github.com/nwjs/nw.js/}\end{center}
  ...
  its own JavaScript file, and there are a number of shared SpaceShark files
...
\begin{table}
  ...
  \begin{center}
    \begin{tabular}{p{0.2\linewidth}p{0.7\linewidth}}
      \toprule
      \textbf{Page Name}                & \textbf{Code artifacts}     \\
      \midrule
      Load File                         & \texttt{assets/js/loadFile.js}\\
      \multirow{2}{*} {Overview}        & \texttt{assets/js/main.js, assets/js/gantt-chart-d3.js, dataTables.js}\\
      \multirow{2}{*} {Cycles}          & \texttt{assets/js/cycles.js, assets/js/gantt-chart-d3.js}\\
      \multirow{2}{*} {Task Statistics} & \texttt{assets/js/runtimeChart.js, assets/js/preemptionChart.js}\\
  ...
  \end{center}
  \vspace{-1em}
  \caption{Code artifacts for each page}
\end{table}
  ...
  charts. This file was originally written by Dimitry Kudrayvtsev,
 ...
 original file is available at \texttt{github.com/dk8996/Gantt-Chart}.
  ...
  placement on the chart by its start time and group. Examples of groups are
  CPU or cycle.

  The chart assigns CSS classes to each rectangle so that they can easily be
  ...
  as well as a property to group on. Behavior on each page varies
...
\begin{itemize}
  \item d3.js (\url{d3js.org})
% d3 - http://d3js.org/
  \item dc.js (\url{dc-js.github.io/dc.js})
% dc - http://dc-js.github.io/dc.js/
  \item Draggabilly (\url{draggabilly.desandro.com})
% draggabilly - http://draggabilly.desandro.com/
  \item Packery (\url{packery.metafizzy.co})
% packery - http://packery.metafizzy.co/
  \item DataTables (\url{datatables.net})
% datatables - https://www.datatables.net/
  \item Twitter Typeahead (\url{twitter.github.io/typeahead.js})
% typeahead - https://twitter.github.io/typeahead.js/
  \item Underscore (\url{underscorejs.org})
%% FIXME - any more?
\end{itemize}
...
\begin{center}
  \url{https://developer.mozilla.org/en-US/docs/Web/API/IndexedDB\_API}
\end{center}
  ...
  \end{itemize}
  These correspond to the JSON types described
  in the Parser section (Section 2.2.3).
...
%  as Local Storage did not have enough space for our large files.
  ...
  display states. One example is the zoom level of the chart, which does not
...
\item \texttt{compareCurrScale}: The last recorded scale for the Compare Tasks
  charts.
...
\item \texttt{cyclesCurrScale}: The last recorded scale for the Cycles charts.	
\item \texttt{cyclesCurrTranslateX}: The last recorded x translate for the
  Cycles charts.
...
\item \texttt{mainCurrScale}: The last recorded scale for the Overview chart.	
\item \texttt{mainCurrTranslateX}: The last recorded x translate for the
  Overview chart.
\item \texttt{maxDuration}: The longest time in seconds that a CPU ran for. Serves as
  the end time for all charts.
\item \texttt{processCurrScale}: The last recorded scale for the Task State
  charts.
...
\begin{center}
  \url{http://www.w3schools.com/Html/html5\_webstorage.asp}
\end{center}
\vspace{-2em}
  \subsection{Selection of Charting Tool} % Rachel
  Many pages render a timeline of running tasks that users can interact
  with by panning and zooming. This chart is critical, as a bug on the rocket manifests
  ...
  \begin{enumerate}
    ...
    \item The events list is filtered to only contain switch events. Switch
      ...
      because it finished.
      Only switch events are needed to determine which task was running, or
    ...
    \item The chart requires a time domain as input, so the endpoint of the
      last event is calculated.
      This calculation depends on which page the user is
   ...
   \item The Gantt chart is passed the events and drawn.
     The chart assigns a CSS class to each task based on its PID, randomly
     ...
     this process was actually quick.

  \end{enumerate}


\chapter{Midyear User Feedback} % Alix
...
\begin{figure}[H]
\begin{center}
\includegraphics[width=.6\textwidth]{oldMain.png}
\caption{A screenshot of our Overview page before winter break. At the top are
several tables of statistics, followed by the chart of running tasks.}
\end{center}
\end{figure}
...
workflow they used in KernelShark as visually finding the problem in the chart,
...
\begin{figure}[H]
\begin{center}
\includegraphics[width=.6\textwidth]{overview-page.png}
...
\begin{figure}[H]
\begin{center}
\includegraphics[width=.6\textwidth]{perProcess-49.png}
...
graphical display of the time series data highlighting the process in the chart.
...
\begin{figure}[H]
\begin{center}
\includegraphics[width=.6\textwidth]{task-state-page.png}
...
view a chart showing how long it was in one of three states: running, waiting,
...
the Overview page chart visualization into cycles.

\begin{figure}[H]
\begin{center}
\includegraphics[width=.6\textwidth]{oldcycles.png}
\caption{A mock-up of the Cycles page.}
\end{center}
\end{figure}
\vspace{-1.5em}
...
markers'', which meant that this functionality would not have worked with 
traces that didn't conform to our specific markers. One engineer
...
varieties of data.
We also added the ability to choose which CPUs are displayed or to remove CPUs if the page becomes cluttered.
\vspace{-.5em}

\begin{figure}[H]
\begin{center}
\includegraphics[width=.6\textwidth]{cycles-page.png}
\caption{A screenshot of the current Cycles page. Checkboxes toggle the display of CPUs.
The input box allows users to choose cycle length.}
\end{center}
\end{figure}
...
Compare Task page because it would allow the user to load multiple charts at once. It should 
...
We received a lot of feedback regarding the charts that are seen on the Overview, Cycles, Task State, and Compare Tasks page, including bug fixes and adding new functionality.
  \begin{itemize}

  \item The labels associated with the charts should be static, and not go off the screen when 
  the chart is zoomed or panned. This feature is especially problematic on the Compare Tasks page, 
  where the user will potentially have large numbers of charts displaying, and it will be 
  important for them to not have to remember what each chart is displaying.
  ...
  \item Users were interested in having the ability to place markers, which 
  is something that KernelShark provided. The user should be able to place one or two
   vertical-line markers onto a chart by clicking. If two markers are placed the user should
    be able to get the duration between the two markers and shift the two markers as one
     unit over the chart, maintaining the same duration between them. The markers could
      potentially be added to the chart at the SVG level, which would likely mean they 
      would pan with the chart as desired, although zooming would require some extra 
      work to keep the markers as a thin line.

  \item Users often had trouble distinguishing where to put their mouse and use the
    scroll wheel to either scroll the page of our tool or to zoom in on the
    chart. One potential fix is to clarify the location of a chart by changing
...
One user suggested that the chart should respond when an event in the events list is clicked.
 This would mirror the existing behavior of clicking on an event in the chart and having
  it scroll in the events list. One possibility for how the chart could respond is adding
   a marker at the timestamp of the event clicked onto the chart. A second possibility is
    translating the chart so that this event is now at the left or the center of
    the chart. Both of these options allow the user to clearly see the event
    within the chart.
  ...
  useful. Combining the two charts to display in the desired order will be 
  difficult. One method that we considered was to have one chart for ``Cycle 0''
  that divides based on CPU, then one for ``Cycle 1'', etc. This solution has the
  downside of producing many charts, which degrades performance, but the user could potentially choose to
  only display the top few cycles, or batch loading could be implemented so that
  charts are only loaded as the user scrolls down.

  One user was interested in filtering the tasks displayed on the Cycles page.
  The tasks list could be filtered and the chart re-rendered with this new tasks
  list.
  ...
  task state chart because the red did not stand out enough against the tan that
  ...
  Currently, the user cannot view which CPU a task is running on in the chart. 
  ...
  could display one chart per CPU that the task is running on, with the 
  process split over multiple charts.
  ...
  is running on. We propose adding a label to the side of each chart as a simple 
  fix. Additionally, the charts should use less vertical space, which would
  allow more charts to fit on the page without loss of 
  ...
  code in \texttt{/assets/js/gantt-chart-d3.js}. Lastly it is not clear to the
   user that they can drag the charts to reorder them. We propose adding a 
   handle icon to the space next to the chart and under the ``Remove Task'' 
   button. This would cue the user to notice that they could reorder the charts 
   by dragging. 
...
\section{Known bugs}
  ...
  \item All charts can be scrolled on the $x$-axis through clicking and dragging until they become no longer visible. While
    they can be dragged back using the white space, this is not good for a user
    experience.

  \item Charts and other images do not resize when the window resizes, unless the
    page is reloaded.

  \item The information displayed by the Task Statistics page, specifically on
    the Runtime section, is incorrect. The bug might be because \texttt{trace-cmd} does not output \texttt{sched\_stat\_runtime} events as
...
\section{Extensions}
  SpaceX developers indicated an interest in interleaving the charts of multiple
  ...
  chart infrastructure and would require an interface for users to choose if they
  want the charts interleaved or not, and which cycle numbers to display.
...
We only want to display two CPU charts at a time on the Cycles page so that both
charts can be clearly visible. In the case that we have more than two CPUs, 
we would hope to be able to paginate the charts. Currently, the charts
...
implemented is batch-loading the list of events on the main page. This allows the user to start interacting with the tool without waiting to load the entire
...
\item {\bf Priority Inversion:} When a high-priority task is indirectly
  prevented from running by something of using a shared resource that the higher
  priority task needs in order to run. 
\item{\bf Process:} A running program. It can have many threads running at once.
\item {\bf Running:} The state of a task when it is being executed on the CPU.
\item {\bf Task:} A single thread of a process.
\item {\bf Waiting:} The state of a task when it cannot run because it is waiting 
  on some piece of information, such as I/O. Synonymous with blocked.
\end{itemize}
