...
\section{Known bugs}
  ...
  \item The information displayed by the Task Statistics page, specifically on
    ...
    \texttt{trace-cmd} not outputting \texttt{sched\_stat\_runtime} events as
    ...
    Statistics page will display incorrect information. If this is not the
...
\section{Extensions}
  ...
  cycle anomalies across CPUs. Making this possible would require tweaking the
  graph infrastructure, and creating an interface for users to choose if they
...
for compressing JSON, such as cjson, but this might cause slowdown when data is
being loaded into the database, as it would need to be decompressed first.
