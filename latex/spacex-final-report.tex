...
and clicking, we are able to find out more information about a specific event.
    ...
    The gantt chart plots all the CPUs from the loaded file onto the same graph.
    It features both double click zooming and mouse scroll zooming. The graph
    can also be horizontally dragged in order to pan.
    Users can click on a specific point in the graph and the table will update its position and jump to that event. 
    
    KernelShark also had graph plots of the CPU's and included similar features as our tool though implemented in different ways. A user could zoom by clicking and dragging a rectangular box, from left to right, over the graph. This allowed the user to precisely view a specific area of the graph that they wanted. Although this zoom functionality seems helpful, it didn't really follow common zoom functionalities of a modern day desktop application. Zooming out of the graph in KernelShark is also unintuitive; the user can click and drag a rectangular box, from right to left, over the graph. This zooming out technique does not give the user any kind of information about how much the graph is zooming out and can therefore be very confusing to users. Despite this, KernelShark has the functionality of being able to click on a spot in the graph and have the table display that event.
...
%% Need to include more information about what specific search functionalities didn't exist in KernelShark?
    The columns of the table consist of the CPU, start time, name, PID, event type, and extra info about a particular event. 
    By default, the table is displayed in ascending order from the first events start time
    to the last events start time. Hovering over any of the table cells highlights that entire row.
    Users can also click on an event to be redirected to that task on the Task State page. 
    The search feature does multi-column filtering based on the search input. 
    The data can be sorted on any column in ascending or descending order by
  clicking the respective column header.
  Below the table, it shows the total number of entries and the range of entries that the user is currently viewing.
  Additionally, the search bar allows for users to search for specific event
  types or task names to minimize what is displayed in the table.

  While KernelShark had a search feature, it did not yield correct search
  results. KernelShark allowed the user to look at a list of tasks and individually plot them on the same graph as the CPUs. To maximize space on the Overview page, we decided to move this feature to its own page (Compare Task Page). 

    KernelShark's main interface looks very similar to the Overview Page, but
    did not perform the same functionality that our tool does, such as correctly
    filtering and searching for events, and allowing users to reorder their
    data.
...
The code for this page is within assets/js/preemptionChart.js and assets/js/runtimeChart.js. The bar graphs use the dc.js dimensional charting library.
  ...
  the preempted the current task. The bottom half is a graph represented the
