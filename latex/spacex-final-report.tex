...
\liaison{Jesse Keller \and Jessica Hester '13 \and Jim Gruen}
...
\normalsize
  Here, the task associated with the event is named ``idle'' and has a PID of
  0. The second field gives the CPU number, and the third the the start time,
  which is measured in milliseconds since kernel boot. The event type, which is
  ...
  event. For the wakeup event, the name, PID, and priority of the task being
  woken up are displayed.
...
\section{Known bugs}
  ...
  \item All graphs can be dragged until they become no longer visible. While
...
\end{itemize}
  ...
  Additionally, there is some refactoring that could be done to make the code
  clearer. For most of the semester, we referred to the overview page as the
  ``main'' page and the task state page as the ``process'' page before adopting
  new labels. These old page titles are still used in our code. As described in
  the parser section of this report, improvements could also be made to its
  structure.
...
\section{more user testing - results}
 We will be conducting more user testing next week and add this to our report
 after that.
