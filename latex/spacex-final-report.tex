...
SpaceX currently uses a tool called KernelShark %%TODO FIXME citation to
visualize trace data and help find any bugs or anomalies
(Figure~\ref{fig:kernelshark}).  SpaceX engineers look for anomalies in process
...
is an inefficient use of engineer's time. \\
...
\end{table}
  ...
  charts. This file was originally written by Dimitry Kudrayvtsev, %% TODO add a reference?
...
One user suggested that the chart should respond when an event in the events list is clicked.
 This would mirror the existing behavior of clicking on an event in the chart and having
  ...
  Currently, the user cannot view which CPU a task is runnnig on in the chart. 
